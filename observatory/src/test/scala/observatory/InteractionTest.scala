...
trait InteractionTest extends FunSuite with Checkers {
  ...
  test("tileLocation of zoom 0, x 0 and y 1") {
    val expectedLocation = Location(-85.0511287798066, -180.0)

    assert(Interaction.tileLocation(0, 0, 1) === expectedLocation)
  }
  ...
  test("tileLocation of zoom 0, x 1 and y 1") {
    val expectedLocation = Location(-85.0511287798066, 180.0)

    assert(Interaction.tileLocation(0, 1, 1) === expectedLocation)
  }
