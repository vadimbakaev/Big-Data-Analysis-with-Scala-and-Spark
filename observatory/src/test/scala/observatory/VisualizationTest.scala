...
trait VisualizationTest extends FunSuite with Checkers {
  ...
  test("greatCircleDistance return ZERO for distance from itself") {
    ...
    assert(greatCircleDistance(from, to) === 0d)
  }
  ...
  test("greatCircleDistance return distance form point to center") {
    ...
    assert(greatCircleDistance(from, to) +- 0.001 === expectedResult)
  }
  ...
  test("interpolateColor between equidistant points") {
    ...
    assert(interpolateColor(points, 0.5d) === Color(50, 50, 50))
  }
  ...
  test("interpolateColor between 2 points") {
    ...
    assert(interpolateColor(points, 1d) === Color(75, 75, 75))
  }
  ...
  test("interpolateColor no greater point") {
    ...
    assert(interpolateColor(points, 1d) === Color(100, 100, 100))
  }
  ...
  test("interpolateColor no lower point") {
    ...
    assert(interpolateColor(points, 1d) === Color(0, 0, 0))
  }
  ...
  test("interpolateColor for 0 distant point greatest point") {
    ...
    assert(interpolateColor(points, 34.65264354993715d) === Color(0, 0, 255))
  }
  ...
  test("interpolateColor for 0 distant point lowest point") {
    ...
    assert(interpolateColor(points, -100) === Color(255, 0, 0))
  }
