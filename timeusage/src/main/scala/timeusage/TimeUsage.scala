...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    (
      columnNames.filter(filed => primaryNeeds.exists(filed.startsWith)).map(col),
      columnNames.filter(filed => workingActivities.exists(filed.startsWith)).map(col),
      columnNames.filter(filed => otherActivities.exists(filed.startsWith) &&
        !primaryNeeds.exists(filed.startsWith) &&
        !workingActivities.exists(filed.startsWith)
      ).map(col)
    )
  }
  ...
  def timeUsageSummary(
    ...
    import spark.implicits._
    val workingStatusProjection: Column = when($"telfs" >= 1d && $"telfs" < 3d, "working").otherwise("not working").as("working")
    val sexProjection: Column = when($"tesex" === 1d, "male").otherwise("female").as("sex")
    val ageProjection: Column = when($"teage" >= 15d && $"teage" <= 22d, "young")
      .when($"teage" >= 23d && $"teage" <= 55d, "active")
      .otherwise("elder").as("age")


    // Create columns that sum columns of the initial dataset
    val minutesInHour = 60d
    val primaryNeedsProjection: Column = (primaryNeedsColumns.reduce(_ + _) / minutesInHour).as("primaryNeeds")
    val workProjection: Column = (workColumns.reduce(_ + _) / minutesInHour).as("work")
    val otherProjection: Column = (otherColumns.reduce(_ + _) / minutesInHour).as("other")
    df
